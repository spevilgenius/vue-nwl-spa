...
@Component
export default class TopNatoPublications extends Vue {
  totalRows = 0
  perPage = 10
  currentPage = 1
  ...
  @reports.State
  public topNatoPubsReportloaded!: boolean
  @reports.State
  ...
  @reports.Action
  public getTopNatoPubsResults!: () => Promise<boolean>

  mounted() {
    this.setHeaderTitle('Top NATO Publications')
    this.$nextTick(function () {
      console.log('TOP PUBS REPORT MOUNTED')
      this.getTopNatoPubsResults()
      this.interval = setInterval(this.waitForIt, 500)
    })
  }
  public waitForIt() {
    if (this.topNatoPubsReportloaded === true) {
      clearInterval(this.interval)
      this.items = this.topNatoPubsReportResults
