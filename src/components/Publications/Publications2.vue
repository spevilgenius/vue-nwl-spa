          ...
          <b-container fluid class="contentHeight m-0 p-0">
            ...
            <b-card no-body>
              <b-tabs class="tabarea" card>
                <b-tab class="vtab all" v-if="!grouped">
                  <template slot="title">
                    ...
                    <b-col cols="12">
                      <b-table responsive striped hover :items="filteredpubs" :fields="fields" primary-key="primarykey" :per-page="perPage" :current-page="currentPage" table-class="table-full" sticky-header table-variant="light" thead-class="blue-500 text-white">
                        ...
                        <template #cell(actions)="data">
                          ...
                          <b-button v-if="currentUser.isLibrarian || currentUser.isNATOLibrarian || currentUser.isActionOfficer" :title="data.item.InDevelopment === 'Yes' ? 'Remove Pub from Dev Status' : 'Place Pub in Dev Status'" variant="white" size="lg" class="actionbutton text-dark" @click="toggleItemDev(data.item)">
                ...
                <b-tab class="vtab grouped" title="Grouped" v-if="grouped">
                  <div v-if="groupedby == 'FSFF'" class="accordion m-1" role="tablist" :style="getStyle('branchaccordion', null)">
                  ...
                  <div v-else class="accordion m-1" role="tablist" :style="getStyle('branchaccordion', null)">
                    <b-card no-body class="m-1" v-for="(item, index) in groupeditems" :key="item">
                      ...
                      <b-collapse :id="'collapse-' + index" accordion="branch-accordion" role="tabpanel">
                        <b-card-body>
                          <b-table responsive striped hover :items="item.items" :fields="fields" primary-key="primarykey" table-class="table-full" sticky-header table-variant="light" thead-class="blue-500 text-white">
                            <template #head()="data">
                              <span class="text-center">{{ data.field.label }}</span>
                            </template>
                            <template #head(actions)> Actions </template>
                ...
                <template #tabs-end>
                  <li v-if="!grouped" role="presentation" class="nav-item nav-btn-export">
                  ...
                  <li v-if="!grouped" role="presentation" class="nav-item nav-btn-filter"><b-button class="btn-filter" variant="success" @click="showFilters">Filter</b-button></li>
                  <li v-if="!grouped" role="presentation" class="nav-item nav-btn-clear"><b-button class="btn-clear-filter" variant="danger" @click="clearFilters" title="Clear filters to show all pubs.">Clear</b-button></li>
                </template>
              </b-tabs>
            </b-card>
          </b-container>
...
export default class Publications extends Vue {
  ...
  @publication.Action
  public returnFunctionalFields!: () => Promise<GroupItem>

  @publication.Action
  public returnFunctionalSeries!: () => Promise<GroupItem>
  ...
  public pubsloaded() {
    ...
    // this.addActivity('<div class="bg-white">' + JSON.stringify(this.approvedpubsforuser) + '</div>')
    this.filterpubs()
  }
  ...
  public filterpubs() {
    ...
    switch (this.filterType) {
      ...
      case 'Meteorology': {
        this.Title = ' > AMETOCPs/AWPs'
        let b = this.filteredpubs
        let c = this.filteredpubs
        let d = this.filteredpubs
        a = a.filter((search) => Vue._.isEqual(search['Prfx'], 'AWP'))
        b = b.filter((search) => Vue._.isEqual(search['Prfx'], 'AMETOCP'))
        a = a.concat(b)
        a = Vue._.orderBy(a, 'Prfx', 'asc')
        this.filteredpubs = a
        break
  ...
  public async groupPubs() {
    this.overlayText = 'Grouping Publications. Please wait.'
    switch (this.groupedby) {
      case 'FSFF': {
        // group by functional series and then functional field
        // items are already filtered so build group items accordingly
        this.groupeditems = []
        let a = this.filteredpubs
        let p: any = {}
        p.type = 'Branch'
        ...
        console.log('GROUPEDITEMS CREATED: ' + this.groupeditems)
        console.log('GROUPITEM CREATED: ' + this.groupitem)
        this.viewReady = true
        break
      }
      case 'FS': {
        // group by functional series
        this.groupeditems = []
        let a = this.filteredpubs
        let response = await this.returnFunctionalSeries()
        let children = response.children
        for (let i = 0; i < children.length; i++) {
          this.groupeditems.push(children[i])
        }
        for (let i = 0; i < this.groupeditems.length; i++) {
          let series = this.groupeditems[i].text
          if (series) {
            let b = Vue._.filter(a, { AdditionalData: { FunctionalSeries: series } })
            if (b && b.length > 0) {
              this.groupeditems[i].count = b.length
              this.groupeditems[i].items = b
            }
          }
        }
        let p: any = []
        for (let i = 0; i < this.groupeditems.length; i++) {
          let count = this.groupeditems[i].count
          if (count && count > 0) {
            p.push(this.groupeditems[i])
          }
        }
        this.groupeditems = p
        console.log('GROUPEDITEMS FS CREATED: ' + this.groupeditems)
        this.viewReady = true
        break
      }
      case 'FF': {
        // group by functional field
        this.groupeditems = []
        let a = this.filteredpubs
        let response = await this.returnFunctionalFields()
        console.log('FUNCTIONAL FIELD RESPONSE: ' + response)
        let children = response.children
        for (let i = 0; i < children.length; i++) {
          this.groupeditems.push(children[i])
        }
        for (let i = 0; i < this.groupeditems.length; i++) {
          // filter the data for this functional field
          let field = this.groupeditems[i].text
          if (field) {
            let b = Vue._.filter(a, { AdditionalData: { FunctionalField: field } })
            if (b && b.length > 0) {
              this.groupeditems[i].count = b.length
        ...
        this.groupeditems = p
        console.log('GROUPEDITEMS FF CREATED: ' + this.groupeditems)
        this.viewReady = true
        break
      }
    }
  ...
  onRouteChanged(val: Route) {
    ...
    this.grouped = this.$route.params !== undefined ? this.$route.params.grouped : false
    this.groupedby = this.$route.params !== undefined ? this.$route.params.groupedby : ''
    this.pubsloaded()
  }
  ...
  public getStyle(element, field) {
    ...
    switch (element) {
      ...
      case 'tablerowarchive':
        style.height = that.contentheight - 400 + 'px'
        style.width = that.contentwidth + 'px'
        break
