      ...
      <b-container class="p-0">
        <b-form>
          ...
          <b-row v-if="superseded" no-gutters>
            <b-col cols="12">
              <!-- <dynamic-modal-select id="dmsSuperseded" v-model="supersededby" :items="archivepubs" :fields="pubsfields" :filter="pubsfilter" title="Select Superceding Publication" label="Superseded By"></dynamic-modal-select> -->
    ...
    <b-row no-gutters class="contentHeight">
      <b-col cols="12" class="m-0 p-0">
        <b-overlay :show="!viewReady" :variant="overlayVariant" class="contentHeight">
          <b-container v-if="searchitems.length > 0" fluid class="contentHeight m-0 p-0">
          ...
          <b-container v-else fluid class="contentHeight m-0 p-0">
            <div class="px300 mx-auto align-middle text-center">NO ITEMS FOUND</div>
          </b-container>
          <template #overlay>
...
export default class SearchResults extends Vue {
  ...
  pageSize = 20
  interval!: any
  searchresults: Array<PublicationItem> = []
  filteredpubs: Array<PublicationItem> = []
  savedfilteredpubs: Array<PublicationItem> = []
  ...
  mounted() {
    ...
    this.$nextTick(function () {
      console.log('MOUNTED QUERY: ' + that.query)
      let data: any = {}
      data.k = this.query
      this.searchAllPublications(data)
      this.interval = setInterval(this.waitForSearch, 500)
    })
  }

  public waitForSearch() {
    if (this.searchitemsloaded === true) {
      clearInterval(this.interval)
      console.log('SEARCH ITEMS RETURNED: ' + this.searchitems.length)
      this.loadstuff()
    }
  }

  loadstuff() {
    // this.setHeaderTitle('Search Results')
    clearInterval(this.interval)
    if (!this.loadingpubs) {
  ...
  public pubsloaded() {
    ...
    if (this.searchitems.length <= 0) {
      // there are no items to show so let the user know
      this.viewReady = true
      /* this.overlayVariant = 'loading'
      this.overlayText = 'No items found.' */
    } else {
      // loop the approved pubs to see if it is in the list of search items and add to searchresults if it is
      for (let i = 0; i < this.searchitems.length; i++) {
        for (let k = 0; k < this.approvedpubsforuser.length; k++) {
          if (this.searchitems[i].DocID === this.approvedpubsforuser[k].DocID) {
            // console.log('SEARCHITEMS: i = ' + i + ', DocID[i] = ' + this.searchitems[i].DocID + ', k = ' + k + ', DocID[k] = ' + this.approvedpubsforuser[k].DocID)
            this.searchresults.push(this.approvedpubsforuser[k])
          }
        }
      }
      this.viewReady = true // TODO: Remove when testing complete
      this.totalRows = this.searchresults.length
      this.filteredpubs = this.searchresults
      this.savedfilteredpubs = this.searchresults
  ...
  public filterpubs() {
    ...
    let a = this.savedfilteredpubs
    // not hiding TACMEMOS or FXP on search
    ...
    switch (this.filterType) {
      case 'complex': {
        ...
        if (hasfilter) {
          ...
          this.$router.push({ name: 'Search Results', params: { query: this.query } })
          /* this.filteredpubs = this.savedfilteredpubs
          this.buildFilters() */
        }
        break
      }
    ...
    if (this.filteredpubs.length < this.searchresults.length) {
  ...
  onRouteChanged(val: Route) {
    ...
    console.log('ROUTE CHANGED VIA WATCH MODULE - SEARCH RESULTS')
    ...
    this.query = this.$route.params !== undefined ? this.$route.params.query : ''
    let data: any = {}
    data.k = this.query
    this.searchAllPublications(data)
    this.interval = setInterval(this.waitForSearch, 500)
  ...
  public clearFilters() {
    ...
    this.$router.push({ name: 'RerouteSimple', params: { route: 'refreshsearch', filterType: '', filterFields: this.fields, query: this.query } })
  }
  ...
  public setfilter(key: string) {
    // set the field to be filtered and then call the complex filter
    for (let j = 1; j < this.fields.length; j++) {
      if (this.fields[j].key === key) {
        this.fields[j].filter = true
      }
    }
    this.$router.push({ name: 'RerouteSimple', params: { route: 'search', filterType: 'complex', filterFields: this.fields, query: this.query } })
  }
